...
#else
static struct videobuf_queue_ops s2226_vb1_ops;
struct s2226_buffer {
	/* common v4l buffer stuff -- must be first */
		 ...
		 unsigned int cmd, unsigned long arg)
{
	...
	case S2226_VIDIOC_STARTDECODE:
	{
		...
		if (res_locked(dev, fh) & RES_STREAM) {
			printk(KERN_INFO "s2226: stream busy stream\n");
			return -EBUSY;
		}
#endif
...
#else
static void s2226_release(struct kref *kref)
#endif
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 39, 0)
	struct s2226_dev *dev = container_of(v4l2_dev, struct s2226_dev,
					     v4l2_dev);
#else
	struct s2226_dev *dev = container_of(kref, struct s2226_dev, kref);
#endif
	int i;
...
#else
	kref_put(&dev->kref, s2226_release);
#endif
...
#ifndef USE_VIDEOBUF2
	videobuf_queue_vmalloc_init(&fh->vb_vidq, &s2226_vb1_ops,
				    NULL, &strm->dev->slock,
				    fh->type,
				    V4L2_FIELD_NONE,
...
#ifndef USE_VIDEOBUF2
	if (res & RES_ENCODE) {
		dprintk(2, "stop encode\n");
		for (i = 0; i < RD_URBS; i++) {
			usb_kill_urb(strm->read_vid[i].urb);
			strm->read_vid[i].ready = 1;
		}
		(void)  s2226_stop_encode(strm->dev, 0);
		videobuf_streamoff(&fh->vb_vidq);
		res_free(strm->dev, fh, RES_ENCODE);
	}
	if (res & RES_PREVIEW) {
		dprintk(2, "stop preview\n");
		for (i = 0; i < RD_URBS; i++) {
			usb_kill_urb(strm->read_vid[i].urb);
			strm->read_vid[i].ready = 1;
		}
		(void)  s2226_stop_preview(strm->dev, 0);
		videobuf_streamoff(&fh->vb_vidq);
		res_free(strm->dev, fh, RES_PREVIEW);
	}
...
/* videobuf1 ops */
static int buffer_setup_vb1(struct videobuf_queue *vq, unsigned int *count,
			unsigned int *size)
{
...
static int buffer_prepare_vb1(struct videobuf_queue *vq, struct videobuf_buffer *vb,
			  enum v4l2_field field)
{
	...
	buf->vb.width = strm->width;
	buf->vb.height = strm->height;
	buf->vb.field = field;
...
static void buffer_release_vb1(struct videobuf_queue *vq,
			   struct videobuf_buffer *vb)
{
	struct s2226_buffer *buf = container_of(vb, struct s2226_buffer, vb);
	dprintk(4, "%s\n", __func__);
...
static struct videobuf_queue_ops s2226_vb1_ops = {
	.buf_setup = buffer_setup_vb1,
	.buf_prepare = buffer_prepare_vb1,
	.buf_queue = buffer_queue_vb1,
	.buf_release = buffer_release_vb1,
};
...
#else
	if (res_locked(fh->dev, fh) & RES_STREAM) {
		dprintk(1, "stream busy, can't set input\n");
		return -EBUSY;
	}
#endif
...
static unsigned int s2226_poll(struct file *file,
...
static int s2226_mmap(struct file *file, struct vm_area_struct *vma)
{
	struct s2226_fh *fh = file->private_data;
...
static int vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	if (!res_get(dev, fh, res)) {
		printk(KERN_ERR "stream busy. streamon\n");
		return -EBUSY;
	}
...
static int vidioc_streamoff(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	if (res & RES_ENCODE) {
		s2226_stop_urbs(fh->strm, 0);
		if (dev->h51_state == STREAM_MODE_ENCODE)
			s2226_stop_encode(dev, 0);
		videobuf_streamoff(&fh->vb_vidq);
		res_free(dev, fh, RES_ENCODE);
	}
	if (res & RES_PREVIEW) {
		s2226_stop_urbs(fh->strm, 0);
		s2226_stop_preview(dev, 0);
		videobuf_streamoff(&fh->vb_vidq);
		res_free(dev, fh, RES_PREVIEW);
	}
	return 0;
}
#endif  /* USE_VIDEOBUF2 */
...
#ifndef USE_VIDEOBUF2
		INIT_LIST_HEAD(&strm->vidq.active);
		strm->vidq.dev = dev;
#endif
...
#if LINUX_VERSION_CODE < KERNEL_VERSION_V4L2DEV
#error "Sorry, kernel version not supported.  Please update kernel to a newer version with more reliable V4L2 libraries. 3.10+ preferably, 2.6.30 at a minimum"
#endif

#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 14, 0)
/*
  #warn "Kernel version 3.14.0 or higher is recommended due to improvements in V4L2 libraries"
*/
#endif
